...
def asymmetric_mse_objective(preds, train_data):
    # Native LightGBM signature: (predictions, dataset)
    labels = train_data.get_label()
    
    # Calculate residual (Actual - Predicted)
    # If residual > 0, the model under-predicted
    residual = labels - preds
    
    # Penalize under-predictions to naturally push the forecasts higher
    # Start with 1.05 (5% extra penalty). If the Kaggle score doesn't move enough, push to 1.10.
    penalty = 1.05 
...
def run_model(train_df, val_df, val_identifiers, features, categoricals):
    ...
    params = {
        'objective': asymmetric_mse_objective,  # <-- Pass the unquoted function name here
        'metric': 'rmse',              
        'learning_rate': 0.03,         
        'num_leaves': 255,             
    ...
    print("      -> Training Model with Custom Asymmetric Loss...")   
    model = lgb.train(
        params, 
        train_data, 
        ...
        # 'fobj' is completely removed from here
        callbacks=[
